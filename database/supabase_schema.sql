...
create table public.owner_settings (
  id uuid default uuid_generate_v4() primary key,
  owner_id uuid references public.owner_profiles(id) on delete cascade unique not null,
  ...
  -- Booking Settings
  auto_accept_bookings boolean default true not null,
  cancellation_policy text default 'flexible' not null,
  booking_buffer_time text default '0' not null,
  
  -- Bank Details (Encrypted or strict RLS in production)
...
create policy "Owners read own settings" on public.owner_settings 
  for select using (owner_id in (select id from public.owner_profiles where user_id = auth.uid()));

-- Owners can insert their own settings
create policy "Owners insert own settings" on public.owner_settings 
  for insert with check (owner_id in (select id from public.owner_profiles where user_id = auth.uid()));

-- Owners can update their own settings
create policy "Owners update own settings" on public.owner_settings 
  for update using (owner_id in (select id from public.owner_profiles where user_id = auth.uid()));
