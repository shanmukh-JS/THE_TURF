...
const navItems = [
  { href: '/player', icon: LayoutDashboard, label: 'Dashboard' },
  { href: '/venues', icon: MapPin, label: 'Explore Turfs' },
  { href: '/player/bookings', icon: CalendarDays, label: 'My Bookings' },
  { href: '/player/favorites', icon: Heart, label: 'Favorites' },
...
export function PlayerSidebar() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`player-sidebar-notifications-${user.id}`)
      .on(
        'postgres_changes',
          ...
          {navItems.map(({ href, icon: Icon, label }) => {
            ...
            return (
              ...
              >
                ...
                />
                <span>{label}</span>
                {href === '/player/notifications' && unreadCount > 0 ? (
