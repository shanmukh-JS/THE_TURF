...
export default function OwnerDashboardPage() {
  ...
  const fetchDashboardData = async () => {
    ...
    if (!venuesData || venuesData.length === 0) { setLoading(false); return }

    setVenues(venuesData)
    const vIds = venuesData.map((v) => v.id)
    setVenueIds(vIds)
    setActiveVenuesCount(venuesData.filter((v) => v.verification_status === 'APPROVED').length || venuesData.length)
  ...
  const insight = useMemo(() => {
    ...
    const todayOccupancy = todaySlots.length > 0
      ? Math.round((todaySlots.filter((s) => s.status === 'booked').length / todaySlots.length) * 100)
      : 0
    ...
    const eveningOccupancy = eveningSlots.length > 0 ? Math.round((eveningBooked / eveningSlots.length) * 100) : 0

    if (todayOccupancy >= 80) return { text: `Today's occupancy is at ${todayOccupancy}%. Great day!`, type: 'positive' }
    if (eveningOccupancy >= 75) return { text: `Evening slots are ${eveningOccupancy}% full. Peak demand detected.`, type: 'positive' }
    if (revenue > 0 && todayRevenue > (yesterdayRevenue || 1)) return { text: `Today's revenue is trending above yesterday. Keep it up!`, type: 'positive' }
    if (todayOccupancy < 30 && todaySlots.length > 0) return { text: `Only ${todayOccupancy}% slots filled today. Consider running a promotion.`, type: 'attention' }
    if (avgRating >= 4.5) return { text: `Your ${avgRating}★ rating is excellent. Customers love your venues!`, type: 'positive' }
    if (totalReviews === 0) return { text: 'No reviews yet. Encourage customers to leave feedback after bookings.', type: 'neutral' }
    return { text: `You have ${uniqueCustomers} unique customers. Focus on repeat visits!`, type: 'neutral' }
  }, [loading, todaySlots, revenue, todayRevenue, yesterdayRevenue, avgRating, totalReviews, uniqueCustomers])
      ...
      <DashboardAnimationItem>
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-sm text-gray-400">
              ...
              <span className="flex items-center gap-1.5">
                ...
                <span className="text-white font-medium">₹{todayRevenue.toLocaleString('en-IN')}</span> earned today
              </span>
              <span className="text-white/10">|</span>
              <span className="flex items-center gap-1.5">
                <Timer className="w-3.5 h-3.5 text-green-400" />
                {nextBookingMins !== null ? (
                  <>Next booking in <span className="text-white font-medium">{nextBookingMins < 60 ? `${nextBookingMins} min` : `${Math.floor(nextBookingMins / 60)}h ${nextBookingMins % 60}m`}</span></>
                ) : (
          ...
          <div className="flex items-center gap-2.5 bg-green-500/10 border border-green-500/20 px-5 py-2.5 rounded-xl shrink-0">
            <div className={`w-2 h-2 rounded-full ${activeVenuesCount > 0 ? 'bg-green-400 animate-pulse' : 'bg-gray-500'}`} />
            <span className={`${activeVenuesCount > 0 ? 'text-green-400' : 'text-gray-400'} text-sm font-medium`}>
            ...
            {recentBookings.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-[320px] text-center px-6">
                ...
                <p className="text-gray-500 text-xs mt-1.5 max-w-xs">
                  When customers book your venues, they'll appear here. Start by creating slots for your venue.
                </p>
              </div>
            ) : (
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/5">
                    <th className="text-left px-6 py-3 text-xs text-gray-500 font-medium">Customer</th>
                    ...
                    <th className="text-right px-6 py-3 text-xs text-gray-500 font-medium">Action</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {recentBookings.map((b) => {
                    const s = (statusConfig[b.status] || statusConfig.PENDING) as { icon: any; color: string; bg: string; label: string }
                    ...
                    return (
                      <tr key={b.id} className="hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td className="px-6 py-4">
                          <span className="text-sm font-semibold text-white">₹{b.amount.toLocaleString('en-IN')}</span>
                        </td>
                        <td className="px-6 py-4">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-5 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Link href="/owner/slots" className="text-xs text-green-400 hover:text-green-300 transition-colors font-medium">
                View all →
              </Link>
            </div>
              ...
              <div className="space-y-1 max-h-[340px] overflow-y-auto pr-1 scrollbar-thin">
                {todaySlots.slice(0, 8).map((s, i) => (
                  <div key={s.id} className="flex items-center gap-3 group">
                    {/* Time */}
                    <span className="text-[11px] font-mono text-gray-500 w-[68px] shrink-0">{s.time}</span>

                    {/* Timeline dot + line */}
                    <div className="flex flex-col items-center shrink-0">
                      <div className={`w-2.5 h-2.5 rounded-full border-2 ${
                        s.status === 'booked' ? 'border-green-400 bg-green-400/30' :
                        s.status === 'blocked' ? 'border-amber-400 bg-amber-400/30' :
                    ...
                    {/* Content */}
                    <div className={`flex-1 py-1.5 px-3 rounded-lg text-xs font-medium transition-all ${
                      s.status === 'booked'
                      ...
                      <div className="flex items-center justify-between">
                        <span>{s.status === 'booked' ? s.customer : s.status === 'blocked' ? 'Blocked' : 'Available'}</span>
                        {s.venueName && <span className="text-[10px] opacity-60">{s.venueName}</span>}
                      </div>
                    </div>
                  </div>
                ))}
                {todaySlots.length > 8 && (
                  <Link href="/owner/slots" className="block text-center text-xs text-green-400 hover:text-green-300 pt-2 font-medium">
          ...
          <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-5 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                { label: 'Create Slots', icon: Plus, href: '/owner/slots', color: 'text-green-400 bg-green-500/10 border-green-500/15 hover:bg-green-500/20' },
                { label: 'Add Venue', icon: MapPin, href: '/owner/venues/new', color: 'text-blue-400 bg-blue-500/10 border-blue-500/15 hover:bg-blue-500/20' },
                { label: 'Revenue', icon: BarChart3, href: '/owner/revenue', color: 'text-amber-400 bg-amber-500/10 border-amber-500/15 hover:bg-amber-500/20' },
                { label: 'Settings', icon: Settings2, href: '/owner/settings', color: 'text-purple-400 bg-purple-500/10 border-purple-500/15 hover:bg-purple-500/20' },
              ].map((a) => {
                const Icon = a.icon
                return (
                  <Link
        ...
        <DashboardAnimationItem>
          <div className={`rounded-2xl border p-4 flex items-center gap-4 ${
            ...
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
              ...
              insight.type === 'attention' ? 'bg-amber-500/15' :
              'bg-white/5'
            }`}>
              <Lightbulb className={`w-5 h-5 ${
                insight.type === 'positive' ? 'text-green-400' :
                insight.type === 'attention' ? 'text-amber-400' :
                'text-gray-400'
              }`} />
            </div>
            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-0.5">Business Insight</p>
          ...
          >
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <InfoBlock label="Amount" value={`₹${selectedBooking.amount.toLocaleString('en-IN')}`} />
...
function StatCardEnhanced({
  ...
  const accentMap: Record<string, string> = {
    green: 'from-green-500/10 to-emerald-500/5 border-green-500/20 hover:border-green-500/50 hover:from-green-500/20',
    blue: 'from-blue-500/10 to-cyan-500/5 border-blue-500/20 hover:border-blue-500/50 hover:from-blue-500/20',
    amber: 'from-amber-500/10 to-orange-500/5 border-amber-500/20 hover:border-amber-500/50 hover:from-amber-500/20',
    purple: 'from-purple-500/10 to-violet-500/5 border-purple-500/20 hover:border-purple-500/50 hover:from-purple-500/20',
  }
  const iconMap: Record<string, string> = {
    green: 'bg-green-500/15 text-green-400 group-hover:bg-green-500/25 group-hover:scale-110 transition-all duration-300',
    blue: 'bg-blue-500/15 text-blue-400 group-hover:bg-blue-500/25 group-hover:scale-110 transition-all duration-300',
    amber: 'bg-amber-500/15 text-amber-400 group-hover:bg-amber-500/25 group-hover:scale-110 transition-all duration-300',
    purple: 'bg-purple-500/15 text-purple-400 group-hover:bg-purple-500/25 group-hover:scale-110 transition-all duration-300',
  }
  ...
  return (
    <div className={`group relative rounded-2xl border bg-gradient-to-br p-5 overflow-hidden transition-all duration-300 ease-out hover:-translate-y-[3px] hover:shadow-xl hover:shadow-black/40 ${accentMap[accent]}`}>
      ...
      <div className={`absolute -bottom-6 -right-6 w-24 h-24 rounded-full opacity-[0.03] z-0 pointer-events-none transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[8] group-hover:opacity-[0.08] ${blobColorMap[accent] || 'bg-current'}`} />
    </div>
  )
}
