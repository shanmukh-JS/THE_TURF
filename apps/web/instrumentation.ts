...
export async function register() {
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    // Avoid running heavy polling / background workers during build phase on Vercel
    ...
    try {
      ...
      // Bootstrap Outbox Processor and Queue Workers in production runtime only
      if (process.env.REDIS_URL && process.env.VERCEL !== '1') {
      ...
      console.warn('[Instrumentation] Telemetry or Worker initialization bypassed:', err)
    }
  }
}
