...
export async function postJournal(
  ...
  try {
    ...
    if (Math.abs(totalDebit - totalCredit) > 0.001) {
      return { success: false, error: `Journal does not balance. Debit: ${totalDebit}, Credit: ${totalCredit}` };
    }
    ...
    if (error) {
      ...
      return { success: false, error: error.message || 'Database error occurred while posting journal.' };
    }
